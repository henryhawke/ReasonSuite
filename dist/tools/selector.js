...
function renderModeCatalog(modeIds) {
    return modeIds
        .map((id) => {
...
function renderRazorCatalog(razorIds) {
    return razorIds
        .map((id) => {
...
export function buildFallback(request, context, modes, razors) {
    ...
    const decisionPath = [];
    const addReason = (id, reason, delta) => {
        if (!available.has(id))
            return;
        scores.set(id, (scores.get(id) ?? MODE_CATALOG[id]?.base ?? 0.1) + delta);
        const store = reasons.get(id) ?? new Set();
        store.add(reason);
        reasons.set(id, store);
    };
    const note = (observation, implication) => {
    ...
    const detect = (pattern, onHit, observation, implication) => {
    ...
    const creative = detect(/brainstorm|idea|innov|option|alternativ|explore|creative|greenfield/, () => {
    ...
    detect(/experiment|test|measurement|metric|data|evidence|validate|trial|prototype/, () => addReason("scientific", "Experiment/test language", 0.42), "Experimentation or evidence gathering requested", "Scientific decomposition to plan falsifiable tests");
    const explain = detect(/explain|justify|rationale|walkthrough|transparent|audit|trace/, () => addReason("self_explain", "Transparency/explanation cues", 0.38), "Transparency or explanation requested", "Self-explanation to make reasoning explicit");
    const code = detect(/code|script|function|regex|compute|calculate|algorithm|typescript|javascript|python|json|simulate|prototype/, () => addReason("exec", "Coding/calculation keywords", 0.5), "Code or computation keywords present", "Execution sandbox to quickly verify computations");
    const razorExplicit = detect(/razor|occam|popper|hanlon|hitchens|sagan|bayesian occam/, () => addReason("razors.apply", "Explicit razor reference", 0.55), "Explicit request for razors", "Use razor screening as the primary move");
    ...
    const primary = modeScores[0] ?? {
        id: modes[0],
        label: MODE_CATALOG[modes[0]]?.label ?? modes[0],
        score: clampScore(MODE_CATALOG[modes[0]]?.base ?? 0.4),
        reason: MODE_CATALOG[modes[0]]?.summary ?? "Default primary mode.",
    };
